...
/*   Updated: 2017/08/09 16:57:35 by svelhinh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_lines(t_sym *cur_sym, t_sect **first_sect)
{
	...
	while (cur_sym)
	{
		...
		{
			cur_sym->type = get_sym_sect_32(cur_sym, first_sect);
...
static void	get_syms(char *ptr, t_sect **first_sect, t_mach_header *header)
{
	uint32_t		i;
	t_symtab_com	*sym;
	t_nlist			*array;
	...
	while (i++ < header->ncmds)
	{
		if (lc->cmd == LC_SYMTAB)
		{
			...
			first_sym = get_syms_32(sym->nsyms, sym->stroff, ptr, array);
			break ;
		}
	...
	*first_sect = current_sect;
	...
	while (tmp[0]++ < header->ncmds)
	{
		if (lc->cmd == LC_SEGMENT)
			get_sects_32(&current_sect, &tmp[1], lc);
		lc = (void *)lc + lc->cmdsize;
	}
