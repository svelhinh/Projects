...
/*   Updated: 2016/02/08 16:30:55 by svelhinh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	mlx_string_put(m->mlx, m->win, m->sw / 2 - 260, m->sh / 2 + 120, 0xff00,
		"Find the exit of the labyrinth in the shortest time");
	mlx_string_put(m->mlx, m->win, m->sw / 2 - 50, m->sh / 2 + 150, 0xffff,
			"R : RETURN");
}
...
void		commands(t_ray *m)
{
	...
	mlx_string_put(m->mlx, m->win, m->sw / 2 - 50, m->sh / 2 + 30, 0xffff00,
			"P -> PAUSE");
	mlx_string_put(m->mlx, m->win, m->sw / 2 - 80, m->sh / 2 + 60, 0xffff00,
			"ESC -> QUIT GAME");
	rules(m);
